...
- `just start` — runs `deps` and `build`, then serves the site via `wrangler pages dev` (Pages Functions included, matching the live edge). Port comes from `$PORT` (set to `8788` in `.env.local`, loaded by direnv).
...
- **Any other path (`/<day>`)** — treats the path segment as the day name (with `-`/`_`/`+` converted to spaces) and renders the answer view. `_redirects` (Cloudflare Pages) falls back to `index.html` so any URL serves the SPA. `/privacy` and `/privacy.html` are reserved and skip the answer pipeline.
...
- Adds `<meta name="robots" content="noindex">` for any day not in the `PRESET_DAYS` set (derived at module load from `data/presets.ts`), so only curated days get indexed.
...
1. `functions/_middleware.ts` — imports the module directly (`import rawPresets from "../data/presets.ts"`); esbuild bundles the TS at deploy time. Used for `noindex` gating.
