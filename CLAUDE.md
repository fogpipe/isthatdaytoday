...
- `date` (MM-DD) + `answer2` — date-conditional answer. If today's UTC MM-DD matches `date`, render `answer`; otherwise render `answer2`. Used by `valentines`, `kanelbullens`, `syttende-mai`. Resolution happens both client-side (`index.html`) and server-side (`_middleware.ts`, `og.png.ts`) and must stay in sync.
